...
- **UI에서 Person 값 직접 편집 가능** (Q3-B 확정)
...
### 전송 간격
- UI에서 실시간 조정 가능 (**제한 없음**, 사용자 자유 설정)
- 기본값: **500ms**
...
- 기본 대상: `host:514` (단일 대상 확정)
- 페이로드: TR 본문 raw bytes (syslog envelope 없음)
- 인코딩: **EUC-KR** (금융 레거시 시스템 호환)
- "성공" 정의: 로컈 송신 호출이 OSError 없이 완료됨
- 예외(OSError 등)는 `stats_panel`에 실패 카운트로 표시

---

## 6. 로그 파일 저장 (Q1-B 확정)

- 전송된 로그를 `.txt` 파일로 **누적 저장** (감사 추적용)
- 파일명: `loggen_YYYYMMDD_HHMMSS.txt` (세션 시작 시각 기준)
- 저장 위치: 실행 디렉토리 `./logs/` 폴더 자동 생성
- 전송 성공 시마다 1줄 append (raw log + newline)
- 세션 종료(중지 버튼 또는 실행 종료) 시 파일 묻기
- 파일 열기/닫기는 `LogFileWriter` 서비스가 담당

---

## 7. Person UI 편집 (Q3-B 확정)

- `StatsPanel` 또는 별도 `PersonEditorDialog`로 5명의 필드 편집 가능
- 편집 단위: Person 1명씩 선택 후 필드별 QLineEdit 표시
- 편집 내용은 메모리에만 유지 (앱 재시작 시 픽스처 기본값으로 복원)
- `PersonEditorDialog`는 M4 마일스톤에서 구현
...
│  - QTextEdit ReadOnly │  - 현재 Person (편집 버튼 연결)  │
│  - 고정폭 폰트        │  - 현재 전문 타입                 │
│  - 최근 500건 유지    │  - 누적 전송 건수                │
│                       │  - 실패 건수                      │
...
│                       │  - 로그 저장 상태 표시               │
├───────────────────────┴─────────────────────────────┤
│  하단: [전송/중지 토글] [복사] [클리어] (Person편집) (우측정렬) │
├─────────────────────────────────────────────────────┤
